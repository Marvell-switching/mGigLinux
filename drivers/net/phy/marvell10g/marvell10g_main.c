...
enum {
	...
	MV_PCS_CSSR1_COPPER_LINK	= BIT(10),
...
static int mv3610_probe(struct phy_device *phydev)
{
	int ret;

	ret = mv3310_probe(phydev);
	if (ret)
		return ret;
	return ret;
}
...
static int mv3310_set_mactype(struct phy_device *phydev, int mactype)
{
	...
	return phy_set_bits_mmd(phydev, MDIO_MMD_VEND2, MV_V2_PORT_CTRL,
				MV_V2_33X0_PORT_CTRL_SWRST | mactype);
}
...
static int mv3310_read_status_copper(struct phy_device *phydev)
{
	...
	if (cssr1 & MV_PCS_CSSR1_COPPER_LINK)
		phydev->link = 1;
...
static int mv3610_match_phy_device(struct phy_device *phydev)
{
	if ((phydev->c45_ids.device_ids[MDIO_MMD_PMAPMD] &
	     MARVELL_PHY_ID_MASK) != MARVELL_PHY_ID_88X3610)
		return 0;
...
static struct phy_driver mv3310_drivers[] = {
	{
		.phy_id		= MARVELL_PHY_ID_88X3610,
		.phy_id_mask	= MARVELL_PHY_ID_MASK,
		.match_phy_device = mv3610_match_phy_device,
		.name		= "mv88x3610",
...
static struct mdio_device_id __maybe_unused mv3310_tbl[] = {
	{ MARVELL_PHY_ID_88X3610, MARVELL_PHY_ID_MASK },
